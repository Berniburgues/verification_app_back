...
const create = catchError(async (req, res) => {
  ...
  await sendEmail({
    ...
    html: `
            ...
            <p><b>Verify your account clicking this link:</b></p>
            <a href=${link}>${link}</a>
        `,
  });
...
const resetPassword = catchError(async (req, res) => {
  ...
  await sendEmail({
    to: email,
    subject: "Reset Password from Users App",
    html: `
            <h1>Reset Password</h1>
            <p><b>Click here to reset your password:</b></p>
            <a href=${link}>${link}</a>
        `,
  });
...
const newPassword = catchError(async (req, res) => {
  ...
  const hashedPassword = await bcrypt.hash(req.body.password, 10);
  const codeFound = await EmailCode.findOne({ where: { code } });
  if (!codeFound) return res.status(401).json({ error: "Invalid code" });
